...
### 泛型通配符
定义：在使用泛型时，可以使用通配符?表示不确定的类型
   * 上限通配符：? extends 父类类型，表示可以使用父类类型及其子类类型
   * 下限通配符：? super 子类类型，表示可以使用子类类型及其父类类型
   * 无限制通配符：?，表示可以使用任何类型
## 数据结构（树）
### 二叉树
1. 定义：每个节点最多有两个子节点的树形数据结构
2. 特点：每个节点有一个左子节点和一个右子节点
3. 遍历方式：
   * 前序遍历：先访问根节点，再访问左子树，最后访问右子树
   * 中序遍历：先访问左子树，再访问根节点，最后访问右子树
   * 后序遍历：先访问左子树，再访问右子树，最后访问根节点
### 二叉搜索树
1. 定义：一种特殊的二叉树，满足左子节点小于根节点，右子节点大于根节点的性质
2. 特点：支持高效的查找、插入和删除操作
3. 应用：常用于实现动态集合
### 平衡二叉树
1. 定义：一种自平衡的二叉搜索树，保证任意节点的左右子树高度差不超过1
2. 特点：通过旋转操作保持平衡
3. 应用：常用于实现高效的查找、插入和删除操作
### 红黑树
1. 定义：一种自平衡的二叉搜索树，每个节点有颜色属性（红色或黑色）
2. 特点：通过颜色属性和旋转操作
3. 应用：常用于实现关联数组和集合
## Set系列集合
特点：无序、不可重复、无索引
...
### LinkedHashSet
1. 底层数据结构：哈希表+双向链表（维护元素的插入顺序）
2. 特点：有序、不可重复、无索引
3. 接口实现：重写hashCode()和equals()方法，确保元素唯一性
### TreeSet
1. 底层数据结构：红黑树
2. 特点：有序、不可重复、无索引
3. 添加元素过程：
...
4. 遍历方法：
   ...
   * EntrySet()把键值对存储到Set集合中，遍历Set集合用getKey()和getValue()方法获取每一个键值对，再通过键值对获取键和值
...
## 方法引用
1. 定义：方法引用是Lambda表达式的一种简化形式，用于直接引用已有的方法，当做函数式接口中抽象方法的方法体
2. lambda表达式格式：参数列表 -> 方法体
3. 方法引用格式：类名或对象名::方法名
   * 引用静态方法：类名::静态方法名
   * 引用成员方法：对象名::成员方法名
   * 引用构造方法：类名::new
